...
//     let resposta = await fetch('/api/filme/');
...
document.querySelector('button').addEventListener('click', async () => {
    ...
    let resposta = await fetch('/api/lista/' + window.location.href.split('/').at(-1), {
        method: 'PUT',
        headers: {
...
document.querySelector('#deletar').addEventListener('click', async () => {
    let resposta = await fetch('/api/lista/' + window.location.href.split('/').at(-1), {method: 'DELETE'});
...
document.querySelector('#lapis-editar').addEventListener('click', () => {
